...
public class TicketCategoryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all ticketCategories").get("/api/ticket-categories").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new ticketCategory")
                        .post("/api/ticket-categories")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"name\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_ticketCategory_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created ticketCategory").get("${new_ticketCategory_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created ticketCategory").delete("${new_ticketCategory_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TicketCategory entity").exec(scn);
