...
public class TicketPriorityGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all ticketPriorities").get("/api/ticket-priorities").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new ticketPriority")
                        .post("/api/ticket-priorities")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"name\": \"SAMPLE_TEXT\"" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_ticketPriority_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created ticketPriority").get("${new_ticketPriority_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created ticketPriority").delete("${new_ticketPriority_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TicketPriority entity").exec(scn);
